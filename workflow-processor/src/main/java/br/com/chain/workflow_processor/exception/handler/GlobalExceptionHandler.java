...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<String> handleProfileClientException(ProfileClientException ex) {
        // Log the exception details for debugging
        log.error("ProfileClientException caught: {}", ex.getMessage());

        // You can customize the response based on the exception.
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(ex.getMessage()); // Or return a more structured error response
    }
